...
class UserFilter extends React.Component {
    ...
    render() {
        ...
        let options = this.props.options.map( (opt, i) => {
            return <option key={i} value={opt.id}>{opt.name} {opt.lastname} | {opt.email}</option>
        });
        ...
        return (<div className='col-xs-12'>
                ...
                <select style={{width:"100%"}} ref='filter' className='select-user form-control gui-input' onChange={this.onSelectOption} value={this.props.selected}>
                    {options}
                </select>
                </div>
            </div>
